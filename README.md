...
Plain Craft Launcher is a Windows desktop Minecraft launcher codebase. This repository contains a Visual Basic .NET WPF solution for Plain Craft Launcher 2.
...
The repository is not a cross-platform .NET project and is not expected to build with `dotnet build` on macOS or Linux.
...
```powershell
git clone https://github.com/jupiternaut/PCL.git
cd PCL
```
...
```text
docs/screenshots/plain-craft-launcher-main-window.png
```
